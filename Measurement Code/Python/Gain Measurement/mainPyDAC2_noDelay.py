...
def testProgram30DAC(voltage_matrix_30):
    """
    ...
    while msg2.find("Received: " + str(expectedInstructions) + " from serial.") == -1:
        msg2 = receiveFromArduino()
    if not (msg2 == "NO VAL"):
        print(msg2)
...
def testProgram12DAC(voltage_matrix_12):
    """
...
def main():
    x = time.time()
    setSerial(115200, "/dev/cu.usbmodem14401", 30) #BAUDRATE, PORTNAME, #DACS (12 or 30)
    # testProgram12DAC() #Uncomment the needed tester!
    # testProgram30DAC()
    x = time.time() - x #setSerial: ~1-1.5 sec // rest: ~0.3 sec
    print("Terminated in " + str(x) + " seconds.")
