...
root = 'C:/Users/smithlab/Documents/Aaron/TWI/Code/Acquisition-Code/Modules'

if platform.system() == 'Windows':
    if '64 bit' in platform.python_compiler():
        WinDLL(root + '/galil/dll/x64/libcrypto-1_1-x64.dll')
        WinDLL(root + '/galil/dll/x64/libssl-1_1-x64.dll')

        _gclib_path = root + '/galil/dll/x64/gclib.dll'
        _gclibo_path = root + '/galil/dll/x64/gclibo.dll'
        _gclib = WinDLL(_gclib_path)
        _gclibo = WinDLL(_gclibo_path)
    else:
        WinDLL(root + '/galil/dll/x86/libcrypto-1_1.dll')
        WinDLL(root + '/galil/dll/x86/libssl-1_1.dll')

        _gclib_path = root + '/galil/dll/x86/gclib.dll'
        _gclibo_path = root + '/galil/dll/x86/gclibo.dll'
        _gclib = WinDLL(_gclib_path)
        _gclibo = WinDLL(_gclibo_path)
